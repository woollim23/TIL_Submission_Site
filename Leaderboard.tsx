...
export default function Leaderboard() {
  ...
  const cumulativeFineData = useMemo(() => {
    ...
    for (const sub of allSubmissions.data) {
      ...
      if (subDate < cumulativeStart || subDate > selectedWeekEnd) continue;
    ...
    const selectedWeekStartForLoop = startOfWeekMonday(selectedWeekEnd);
    ...
    while (wk <= selectedWeekStartForLoop) {
  ...
  }, [participants.data, allSubmissions.data, selectedWeekEnd]);
  ...
  const weekOptions = Array.from({ length: 26 }, (_, i) => ({
    ...
    label: (() => {
      ...
      const start = d.toLocaleDateString("ko-KR", { month: "numeric", day: "numeric" });
